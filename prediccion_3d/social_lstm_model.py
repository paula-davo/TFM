...
class SocialLSTM(tf.keras.Model):
    ...
    def __init__(self, max_peds, obs_len, pred_len, n_features, lstm_units, social_radius,
                 dropout=0.2, rec_dropout=0.0, noise=0.0, loss="mse"):
        super().__init__()
    ...
    def call(self, inputs, training=False):
        # Características, posición 3D en último frame observado, máscara
        ...
        # Aplana peatones: (batch, max_peds, obs_len, n_features) -> (batch*max_peds, obs_len, n_features)
        ...
        # Filtros: in_radius (1 si está en el radio de vecindad), 
        # diag_mask (máscara de vecinos: uno no es vecino de si mismo),
    ...
    def _masked_mse(self, y_true, y_pred, mask):
        # Calcula la pérdida teniendo en cuenta la máscara de peatones reales en la escena.
        # loss_name="mse" -> error cuadrático; "mae" -> error absoluto
        err = tf.square(y_true - y_pred) if self.loss_name == "mse" else tf.abs(y_true - y_pred)
        # Error medio por peatón
        err = tf.reduce_mean(err, axis=[2, 3])
        ...
        return tf.reduce_sum(err * mask_f) / (tf.reduce_sum(mask_f) + 1e-8)

    def train_step(self, data):
        # Carga los datos: características, última posición 3D observada, máscara
