"""
Modelo LSTM que recibe deteccciones 3D reales del lidar en metros -> entrada de 9 características.
Permite medir la robustez del predictor al ruido del detector, en comparación con el modelo entrenado
con etiquetas 3D (techo ideal).
"""
...
class Lidar3DDetTrainer:

    """
    Lidar3DDetTrainer entrena el modelo LSTM con detecciones 3D (realizadas por el sensor lidar)
    """
    ...
    def load_data(self):
        # Carga los datos -> detecciones 3D reales en metros y aplica la partición oficial
        X = np.load(f"{self.data_dir}/X3d_det_motion.npy")
        Y = np.load(f"{self.data_dir}/Y3d_det.npy")
        subtrack_ids = np.load(f"{self.data_dir}/subtrack_ids_ds.npy", allow_pickle=True)
        print(f"X(3D detecciones): {X.shape}")
    ...
    def train(self):
        ...
        cbs = [EarlyStopping(monitor="val_loss", patience=10, restore_best_weights=True),
               ...
               ModelCheckpoint(f"{self.data_dir}/prediccion_3d/best/lidar3d_det_best.keras", monitor="val_loss", save_best_only=True)]
    ...
    def save_outputs(self):
        ...
        np.save(f"{self.data_dir}/prediccion_3d/history/hist_lidar3d_det.npy", self.history.history)
        ...
        plt.title("Curva de entrenamiento — LiDAR 3D detecciones (solo-mov)")
        plt.legend(); plt.grid(alpha=0.3); plt.tight_layout()
        plt.savefig(f"{self.data_dir}/prediccion_3d/figuras/loss_lidar3d_det.png", dpi=130); plt.close()
        print("Guardado: figuras/loss_lidar3d_det.png, hist_lidar3d_det.npy")

    def evaluate(self):
        # Predicción de desplazamientos relativos (en metros)
        pred = self.model.predict(self.X_va_s, batch_size=512, verbose=0)
        # Métricas: modelo LSTM con detecciones 3D vs CV baseline vs baseline quieto (desplazamiento futuro nulo)
        ...
        # Comparación: LSTM con detecciones 3D vs quieto vs CV
        print("\n===== LiDAR 3D con DETECCIONES reales, solo-movimiento (METROS) =====")
        ...
        print(f"{'3D det solo-mov':<28} {ade:>8.4f} {fde:>8.4f} {(1-ade/ade_cv)*100:>7.1f}% {(1-fde/fde_cv)*100:>7.1f}%")
