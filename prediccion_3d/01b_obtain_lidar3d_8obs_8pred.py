...
if __name__ == "__main__":
    X = np.load(f"{DATA_DIR}/data/X3d_motion.npy")   # (N, 8, 9) observación (etiquetas),
    Y = np.load(f"{DATA_DIR}/data/Y3d.npy")          # (N, 12, 2) desplazamiento futuro
    ...
    np.save(f"{DATA_DIR}/data/X3d_motion_8obs_8pred.npy", Xb)
    np.save(f"{DATA_DIR}/data/Y3d_8obs_8pred.npy", Yb)
