...
if __name__ == "__main__":
    X = np.load(f"{DATA_DIR}/data/X3d_det_motion.npy")   # (N, 8, 9) observación (detecciones)
    Y = np.load(f"{DATA_DIR}/data/Y3d_det.npy")          # (N, 12, 2) desplazamiento GT
    ...
    np.save(f"{DATA_DIR}/data/X3d_det_8obs_8pred.npy", Xb)
    np.save(f"{DATA_DIR}/data/Y3d_det_8obs_8pred.npy", Yb)
